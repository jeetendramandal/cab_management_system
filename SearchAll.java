...
class W_Driver extends JFrame
{
	...
	public W_Driver(String str)
	{
	   setTitle("Search Result");
		...
		try
		{
			String[] colHeads={"ID","NAME","ADDRESS","PHONE NUMBER","DATE_JOIN"};
	                str="Driver";
...
class N_W_Driver extends JFrame
{
    ...
    JLabel JLBanner = new JLabel("Search Results");
    JLabel JLPic = new JLabel(new ImageIcon("images/bSearch.png"));
	JLabel StatusLabel = new JLabel("CREATED BY AKSHAY AND PRITAM",JLabel.CENTER);
...
class Empl extends JFrame
{
	...
	public Empl(String str)
	{
		...
		try
		{
			String[] colHeads={"ID","NAME","ADDRESS","PHONE NUMBER","PROCESS"};
	                str="Employees";
			string= "SELECT * FROM "+str;
			Class.forName("org.gjt.mm.mysql.Driver");
			c = DriverManager.getConnection("jdbc:mysql://localhost/project","root","vignet");
